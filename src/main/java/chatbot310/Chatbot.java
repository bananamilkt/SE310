...
public class Chatbot {
	...
	public String getResponse(String userInput) {
		return initialBotResponse(processInput(userInput));
	}
	private String initialBotResponse(String sentence) {
		return "TechSupport: "+sentence;
	}
	/**
	...
	public String processInput(String userInput){	
		...
		if(current_node.hasFlow() && !flowComplete) {
			returnMessage=returnMessage+processFlow(userInput);	
			...
			if (current_node.getFlow().title.equals("Review")) {
				...
				String flowOut=current_node.getFlow().saveInputs(flowInputs);
				//gui.sendMessage("Review saved successfully. Thank you for your feedback!");
				returnMessage=returnMessage+"Review saved successfully. Thank you for your feedback!"+"/n"+flowOut;
			}
			if (current_node.getFlow().title.equals("Store")) {
				//gui.userinput.setEditable(false);
				//gui.userinput.setText("Agent processing purchase...");
				String flowOut=current_node.getFlow().processingPurchase(flowInputs);
				//gui.sendMessage("Order has been placed successfully. Thanks for choosing Oculus!");
				returnMessage=returnMessage+"Order has been placed successfully. Thanks for choosing Oculus!"+"/n"+flowOut;
			}	
			if (selectPath(current_node, userInput, 0) == null) { // No more paths, back to Root
				...
				returnMessage=returnMessage+"Thanks for using our support bot. The end of this branch has been reached. Returning to top."+current_node.getMessage()+"/n";
			} else {
				current_node = current_node.getChildren().get(0); // Proceed to first child after Flow end (If there is one)
				//gui.sendMessage(current_node.getMessage());	
				returnMessage=returnMessage+current_node.getMessage()+"/n";
			}
			...
			if (potentialPath != Root) { // If previous input resulted in potentialPath, check if user says yes or no
				if (userInput.toLowerCase().contains("yes")) {
					...
					returnMessage=returnMessage+"Thanks for letting me know. Changing branch..."+"/n";
					current_node = potentialPath;
				} else {
					//gui.sendMessage("Thanks for letting me know. Can you please try me again with different words?");
					returnMessage=returnMessage+"Thanks for letting me know. Can you please try me again with different words?"+"/n";
				}
				potentialPath = Root;
				...
				if (starting_node == current_node) {
					...
					if (potentialPath != Root) {
						...
						returnMessage=returnMessage+"I'm sorry, I don't recognise your input for the current branch. Did you want information on \"" + potentialPath.getTitle() + "\" instead?/n";
						...
						returnMessage=returnMessage+"I'm sorry, I don't recognise your input for the current branch. Can you try me again with different words?"+"/n";
					}
				}
			}
	...
	private String processFlow(String userInput) {
		...
		if (flowMarker == -1) { // Sends initial sentence
			...
			returnMessage="Hmm. The input you have entered doesn't seem to be right. Please try again.";
		} else {
			flowMarker++;
			//gui.sendMessage(currentFlow.responses.get(flowMarker) + "\n");
			returnMessage=currentFlow.responses.get(flowMarker);
		...
		if (flowMarker >= currentFlow.pattern_format.size()) { // Checks if end of flow has been reached
			flowMarker = -1;
			flowComplete = true;
			returnMessage=returnMessage+processInput(null);
		}
		return returnMessage;
	}
	/**
	...
	private void initializeFlows() {
		InputStream path = this.getClass().getClassLoader().getResourceAsStream("/Flows.txt");
		//InputStream path = getClass().getClassLoader().getResourceAsStream("Flows.txt");
		if (path == null) {
	...
	void initializeResponses() {
		System.out.println(getClass().getClassLoader().getResource("/dict").getFile());
		int lineError = 0;	
		String whole = null;	
		try {// Main method. Recursively calls down to implement parent-child tree
			InputStream path = getClass().getClassLoader().getResourceAsStream("/Responses.txt");
			if (path == null) {
